    ...
    context('when the author creation succeeds', () => {
      ...
      beforeEach(async () => {
        ...
        response = await request(app)
          .post('/authors/new')
          .type('form')
          .send({ 'name': 'JJRousseau', 'pseudo': 'JJR', 'email': 'jjr@exemple.net', 'language':'french' })
          .redirects(0)
      })

      it('should call the service with author data', () => {
        // then
        expect(authorService.create).to.have.been.calledWith({ email: 'jjr@exemple.net', name: 'JJRousseau', pseudo: 'JJR', language: 'french' })
      })
